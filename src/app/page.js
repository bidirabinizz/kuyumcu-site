...
export default function Home() {
  ...
  // Helper to render appropriate SVGs based on semantic analysis of titles
  const getIcon = (title) => {
    const t = title.toUpperCase();
    if (t.includes('KATALOG')) {
      return (
    ...
    if (t.includes('DÖVİZ') || t.includes('ALTIN') || t.includes('KURLAR')) {
      return (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    if (t.includes('IBAN') || t.includes('BANKA') || t.includes('HESAP')) {
      return (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    if (t.includes('SHOWROOM') || t.includes('MAĞAZA') || t.includes('ADRES') || t.includes('KONUM')) {
      return (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    if (t.includes('TELEFON') || t.includes('İLETİŞİM') || t.includes('ARAT') || t.includes('ARA:')) {
      return (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    if (t.includes('E-POSTA') || t.includes('MAİL')) {
      return (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  // Handler to render actual interactive link actions
  const renderLinkButton = (link) => {
    const classes = `link-item ${link.is_featured ? 'link-item-featured' : ''}`;
    
    // In-app actions mappings
    if (link.url === 'whatsapp-siparis') {
      return (
        <a 
          key={link.id}
          href={`https://wa.me/${settings.whatsapp_number}?text=Merhaba%20Çapar%20Kuyumculuk,%20bilgi%20ve%20sipariş%20almak%20istiyorum.`}
          target="_blank"
          rel="noopener noreferrer"
          className={classes}
        >
          <div className="link-icon-wrapper">{getIcon(link.title)}</div>
    ...
    // Default external mapping (tel:, mailto:, map link, social, etc.)
    return (
      <a 
        key={link.id}
        href={link.url}
  ...
  return (
    <div className="layout-wrapper">
      <main className="main-container">
        ...
        <a 
          href={`https://wa.me/${settings.whatsapp_number}?text=Merhaba%20Çapar%20Kuyumculuk,%20hızlı%20sipariş%20vermek%20istiyorum.`}
          target="_blank"
          rel="noopener noreferrer"
        ...
        >
          <div style={{
            ...
            <div style={{ marginTop: '0.8rem', textAlign: 'center' }}>
              <span style={{ color: '#d4af37', fontFamily: 'var(--font-serif)', fontSize: '1.2rem', fontWeight: 'bold', letterSpacing: '1px' }}>
                HIZLI SİPARİŞ HATI
              </span>
              <p style={{ color: '#ffffff', fontSize: '0.75rem', marginTop: '0.2rem', opacity: 0.8 }}>
                7/24 WhatsApp Üzerinden Sipariş ve Bilgi Alın
              </p>
            </div>
          </div>
        </a>
        ...
        <section className="bottom-info-section">
          ...
          <p className="info-text">
            Çalışma Saatleri: <span className="info-hours">{settings.working_hours}</span>
          </p>
        </section>
